...
import {getOrgsRepositories, getUsersRepositories, UserRepository} from '../utility/oktokitHelper'
...
describe('Sorting Repositories', () => {
	test('Sort By Full Name Asc', async () => {
		...
		const inOrder = isArrayInAscendingOrder(
			data,
			(rep: UserRepository ) => rep.full_name.toLowerCase(),
		)
	...
	test('Sort By Created Asc', async () => {
		...
		const inorder = isArrayInAscendingOrder(
			...
			(prevTime: string, currTime: string) => {
				return new Date(currTime).getTime() >= 
