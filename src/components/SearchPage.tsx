...
const SearchPage = () => {
  ...
  const [serchResult, setSearchResult] = useState<
    ListOrgReposResponse | ListUserReposResponse | undefined
  >();
  ...
  const { storage: historyData, store: setHistory } = useLocalStorage<
    SearchQuery[]
  >("history", []);

  const sortingData = useRef<SortingData>({
    direction: "desc",
    type: "created",
  });
  ...
  const search = async () => {
    ...
    const response =
      searchType === "user"
        ? await getUsersRepositories(
            ...
            sortType,
            sortDirection,
            isFilterableFieldForUsers(filterBy.current)
              ? filterBy.current
              : undefined
          )
        : await getOrgsRepositories(
            ...
            sortType,
            sortDirection,
            isFilterableFieldForOrgs(filterBy.current)
              ? filterBy.current
              : undefined
          );
    ...
    setFetching(false);
    const duplicate = existInHistory(historyData, {
      name: searchQuery,
      type: searchType,
    });
    if (!duplicate) {
      setHistory([...historyData, { name: searchQuery, type: searchType }]);
    }
  ...
  const errorMessageJsx = !!serchResult?.errorMessage && (
    <VStack
      h={"100%"}
  ...
  const tableJsx = !!serchResult?.data && (
    <RepositoryList
        ...
        <HStack w={"80%"}>
          <InputGroup w={"60%"}>
            ...
            <InputRightElement cursor="pointer" onClick={newSearch}>
              <CiSearch data-testid={"searchButton"} />
            </InputRightElement>
          </InputGroup>
